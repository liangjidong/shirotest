...
import shiro.util.JdbcTemplateUtils;
...
public class UserDaoImpl extends JdbcDaoSupport implements UserDao {
    ...
    public User createUser(final User user) {
        final String sql = "insert into sys_users(username, password, salt, email,locked) values(?,?,?,?,?)";
        ...
        getJdbcTemplate().update(new PreparedStatementCreator() {
            ...
            public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                ...
                psst.setString(4,user.getEmail());
                psst.setBoolean(5, user.getLocked());
                return psst;
            }
        }, keyHolder);
    ...
    public void updateUser(User user) {
        String sql = "update sys_users set username=?, password=?, salt=?, email=?,locked=? where id=?";
        getJdbcTemplate().update(sql, user.getUsername(), user.getPassword(), user.getSalt(), user.getEmail(),user.getLocked(), user.getId());
    }

    @Override
    public void updateUserToActive(Long userId) {
        String sql = "update sys_users set actived=1 where id=?";
    ...
    public User findOne(Long userId) {
        String sql = "select id, username, password, salt,email,actived,locked from sys_users where id=?";
    ...
    public User findByUsername(String username) {
        String sql = "select id, username, password, salt,email,actived,locked from sys_users where username=?";
    ...
    public User findByEmail(String email) {
        String sql = "select id, username, password, salt,email,actived,locked from sys_users where email=?";
        List<User> userList = getJdbcTemplate().query(sql, new BeanPropertyRowMapper(User.class), email);
        if (userList.size() == 0) {
            return null;
        }
        return userList.get(0);
